...
<main id="main">
  <section class="hero shell">
    <div class="hero__copy">
      ...
      <p class="hero__lede">GTVAFRIK is the media, marketing and advocacy platform helping African businesses, startups and change-makers get seen worldwide.</p>
  ...
  <section class="section shell" id="services">
    <div class="what-intro-grid"><div class="what-intro-grid__copy"><p class="eyebrow">/ What we do</p><h2>Not just a message.<br><span class="accent">A movement with momentum.</span></h2><p>We work where culture, media and impact meet, building the strategy, stories and platforms that carry ideas further.</p></div><figure class="what-intro-grid__video"><video autoplay muted loop playsinline controls preload="metadata" src="https://gtvafrik.com/wp-content/uploads/2026/09/WHAT-WE-DO-SECTION-VIDEO.mp4"><a href="https://gtvafrik.com/wp-content/uploads/2026/09/WHAT-WE-DO-SECTION-VIDEO.mp4">Watch the What We Do video</a></video></figure></div>
  ...
  <section class="section shell" id="work">
    ...
    <div class="proof-grid"><?php foreach ($proof_items as $item) : ?><button class="proof-card proof-card--<?php echo esc_attr($item[5]); ?>" type="button" data-proof-type="<?php echo esc_attr($item[3]); ?>" data-proof-src="<?php echo esc_url($item[4]); ?>" data-proof-title="<?php echo esc_attr($item[1]); ?>"><?php if ($item[3] === 'youtube') : ?><span class="proof-card__preview" aria-hidden="true"><img src="https://i.ytimg.com/vi/dK0AzE0KYjI/hqdefault.jpg" alt=""></span><?php elseif ($item[4]) : ?><span class="proof-card__preview" aria-hidden="true"><video muted loop playsinline preload="metadata" src="<?php echo esc_url($item[4]); ?>"></video></span><?php endif; ?><span class="proof-card__shade" aria-hidden="true"></span><span class="proof-card__case">Case / <?php echo esc_html($item[0]); ?></span><span class="proof-card__shape" aria-hidden="true"></span><span class="proof-card__meta"><?php echo esc_html($item[2]); ?></span><strong><?php echo esc_html($item[1]); ?></strong><small>Watch the work <b aria-hidden="true">▶</b></small></button><?php endforeach; ?></div>
  </section>

  <section class="section shell" id="clients">
    <div class="brand-intro"><p class="eyebrow">/ Who we've built for</p><div><h2>Across sectors.<br><span class="accent">Across the continent.</span></h2><p>From boardrooms to newsrooms and stages, we've carried the signal for businesses, institutions and startups who needed the world to pay attention.</p><p class="brand-intro__link">Working with your brand? <a href="#contact">Let's talk.</a></p></div></div>    <?php $brands = [
    ...
    ]; $opaque_logos = ['APC-All-Progressives-Congress.png','HyperCITY.jpg','NCDC-North-Central-Development-Commission.jpg','NEXCO-Elevators.jpg','PDP-Peoples-Democratic-Party.png']; ?><div class="brand-logo-marquee"><div class="brand-logo-track"><?php for ($logo_copy = 0; $logo_copy < 2; $logo_copy++) : ?><div class="brand-grid"<?php echo $logo_copy ? ' aria-hidden="true"' : ''; ?>><?php foreach ($brands as $brand) : $logo = gtvafrik_media_url($brand[0]); ?><div class="brand-tile<?php echo in_array($brand[0], $opaque_logos, true) ? ' brand-tile--opaque' : ''; ?>"><?php if ($logo) : ?><img src="<?php echo esc_url($logo); ?>" alt="<?php echo $logo_copy ? '' : esc_attr($brand[1]) . ' logo'; ?>" loading="lazy"><?php else : echo esc_html($brand[1]); endif; ?></div><?php endforeach; ?><div class="brand-tile brand-tile--text"><strong>TMG</strong><small>Tinubu Mega Group</small></div></div><?php endfor; ?></div></div>
  </section>

  <section class="trust"><div class="shell trust__grid"><div><p class="eyebrow">/ Trust, with receipts</p><h2>Built to travel.<br>Trusted to land.</h2></div><div class="trust__copy"><p>The best work is generous: it makes room for more voices, gives partners a clear point of view and leaves people with something to do next.</p><ul><li>Media</li><li>Culture</li><li>Commerce</li><li>Impact</li><li>Diaspora</li></ul></div><?php $trust_icon = gtvafrik_media_url('AFRICAN ICON - YELLOW TRUSTED SECTION.png'); if ($trust_icon) : ?><figure class="trust__art"><img src="<?php echo esc_url($trust_icon); ?>" alt="Colourful patterned map of Africa" loading="lazy"></figure><?php endif; ?></div></section>
  ...
  <section class="section shell contact" id="contact"><div class="contact__grid">
    <div><p class="eyebrow">/ Start a conversation</p><h2>Let's put your<br>story <span class="accent">on air.</span></h2><p>Have a brief, a big question or just a spark? Send it through. We'll come back with a point of view.</p><div class="contact__links"><a class="contact__email" href="mailto:info@gtvafrik.com">info@gtvafrik.com</a><a class="button button--pill contact__whatsapp" href="https://wa.me/2348188059300" target="_blank" rel="noopener">WhatsApp <span aria-hidden="true">↗</span></a></div></div>
