(function(){
  ...
  document.querySelectorAll('.proof-card').forEach(function(card){var trigger=card.matches('button')?card:card.querySelector('.proof-card__watch');if(!trigger)return;trigger.addEventListener('click',function(){var stage=proofModal.querySelector('.media-modal__stage');proofModal.querySelector('h2').textContent=card.dataset.proofTitle;if(card.dataset.proofType==='youtube'){stage.innerHTML='<iframe title="'+card.dataset.proofTitle+'" src="'+card.dataset.proofSrc+'" allow="autoplay; encrypted-media; picture-in-picture" allowfullscreen></iframe>';}else{stage.innerHTML='<video controls autoplay playsinline><source src="'+card.dataset.proofSrc+'"></video>';}proofModal.showModal();});});
